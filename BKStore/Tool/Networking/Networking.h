/**
 ...
 -  网络请求类，包含，Get单个请求，Get单个请求含有进度回调，Get批量请求。 Post单个请求，Post单个请求含有进度回调，Post批量请求。普通单个上传图片的任务和批量上传图片的任务。检查网络连接状态。
 */
...
typedef void (^CompletionBlock)(NetworkModel *model, NSString *netErr);
